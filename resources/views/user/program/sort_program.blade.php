    ...
    <section class="bg-primer h-16 py-3 z-[100] fixed top-0 left-0 right-0 w-full">
        <div class="flex justify-start h-full container">
            ...
            <div class="container h-full lg:max-w-lg flex items-center">
                <h4 class="text-white font-semibold text-base">Pilih kategori program</h4>
            </div>
        </div>
    </section>

    <section class="bg-white flex-1 pt-16 pb-4">
        <form action="{{ route('program.sort_by') }}" class="container">
            <!-- Kategori 1 -->
            <button id="urgent" class="w-full flex items-center justify-between py-4 border-b">
                <div class="flex items-center">
                    <span class="mx-5 text-left text-sm font-semibold text-[#4A4A4A]">Paling Mendesak</span>
                </div>
            ...
            <!-- Kategori 2 -->
            <button id="latest" class="w-full flex items-center justify-between py-4 border-b">
                <div class="flex items-center">
                    <span class="mx-5 text-left text-sm font-semibold text-[#4A4A4A]">Terbaru</span>
                </div>
                <div class="grid place-items-center justify-sel-end">
                    <svg width="16px" height="16px" viewBox="0 0 24 24" fill="none"
                        xmlns="http://www.w3.org/2000/svg" role="img">
    ...
    <script>
        $(document).ready(function () {
            $('#urgent').on('click', function () {
                $('input[name=sort_by]').val('urgent');
            });
            $('#latest').on('click', function () {
                $('input[name=sort_by]').val('latest');
            });
        });
    </script>
</body>
